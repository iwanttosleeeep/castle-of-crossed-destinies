...
- 用户逐条修改、删除、补充和确认抽取事实
- 三种可切换的报告模式：轻量人格 Skill、纯 API 与 Grounded Skills 引用审计
...
- 用户提问后：独立回答 → 每室一次 rebuttal → 主持人结案总结；Grounded 模式另有定向 challenge
...
- **轻量人格 Skill（默认）**：每间 chamber 只进行一次自然语言调用，仅加载
  `skills/guided-chamber-reading/` 中的短防幻觉契约和对应人物设定；不加载完整知识规则，
  不要求 JSON、Evidence ID 或 Rule ID。它保留纯 API 模式的速度与长文能力，同时让
  七间密室拥有不同声音。用户提问后，同一轻量 Skill 继续负责各室独立回答、阅读其他
  第一轮证词后各作一次 rebuttal，最后由自然语言主持人总结，全程不解析模型正文 JSON。
- **实验自由模式**：每间 chamber 只进行一次自然语言调用，不读取项目
  Skills，不要求 JSON、Evidence ID 或 Rule ID；随后用一次自然语言调用生成
  Tribunal。它是最接近普通聊天式分析的对照组。
- **Grounded Skills**：保留七套完整 Skills、规则白名单、证据引用、人物语气隔离、
  结构化 Tribunal 与一轮制交叉质询。它的调用次数更多，并可能拒绝未通过引用校验的
  内容。

轻量 Skill 模式开放自然语言质询与 rebuttal；纯 API 只作为无 Skill 对照组，不开放
质询。两者都没有逐条引用审计。切换模式不会删除原有 Skills 或已确认事实，案件恢复后
可直接重新生成，无需重新上传。
