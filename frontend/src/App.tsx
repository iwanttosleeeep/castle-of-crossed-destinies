...
export default function App() {
  ...
  return <main>
    ...
    {stage==='report' && caseData && <ReportView data={caseData} token={caseToken} providerHeaders={providerHeaders} setData={setCaseData} review={()=>{setStage('review');scrollTo('review')}} apiKey={apiKey} setApiKey={setApiKey} loading={loading} setLoading={setLoading} error={error} setError={setError}/>}
...
function ReportView({data,providerHeaders,setData,review,apiKey,setApiKey,loading,setLoading,error,setError}:{data:CaseData;token:string;providerHeaders:()=>Record<string,string>;setData:(d:CaseData)=>void;review:()=>void;apiKey:string;setApiKey:(v:string)=>void;loading:string;setLoading:(v:string)=>void;error:string;setError:(v:string)=>void}) {
