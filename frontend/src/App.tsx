...
export default function App() {
  ...
  return <main>
    ...
    <section className="hero"><p className="eyebrow">SEVEN LOCAL SKILLS · AUDITABLE BY DESIGN</p><h1><em>THE CASTLE OF<br/>CROSSED DESTINIES</em></h1><p className="lede">一份出生资料，七套独立 Skill 证词。它们只解释你提供的盘面事实；每个结论都必须接受证据、矛盾与宽泛性审计。</p><div className="rule"/><p className="note">No external APIs. No hidden chart calculation. Zi Wei is back in the castle.</p><div className="castle-card"><img src="/castle-card.jpg" alt="The Castle of Crossed Destinies card"/></div></section>
    ...
    {report && <ReportView report={report}/>}<section className="method" id="method"><p className="eyebrow">THE METHOD</p><h2>Facts first. Skills second.</h2><p>不调用 API，也不在后台偷算盘。每间 chamber 只读取事实档案中属于自己的条目；Skill 的每条 claim 必须回指事实 ID。Tribunal 再比较这些可追溯证词。</p></section><footer>THE CASTLE OF CROSSED DESTINIES <span>Not advice. Not proof. A traceable conversation.</span></footer>
  </main>
}
