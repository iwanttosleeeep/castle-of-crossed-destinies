...
type ChamberReport = { system_id:string; display_name:string; mode?:'free'|'grounded'; free_text?:string|null; claims:Claim[]; sections:Record<string,string[]>; abstentions:string[]; warning?:string|null }
...
type CaseData = { case_id:string; profile:any; systems:string[]; status:string; report_mode?:'free'|'grounded'; extractions:Record<string,Extraction>; confirmed_facts:Record<string,Fact[]>; reports:Record<string,ChamberReport>; tribunal?:{mode?:'free'|'grounded';findings:Finding[];summary:string;counts:Record<string,number>;disclaimer:string}|null; debates:Hearing[]; report_warnings?:string[] }
...
export default function App() {
  ...
  const [reportMode, setReportMode] = useState<'free'|'grounded'>('free')
  ...
  async function restoreCase(event:FormEvent) {
    ...
    try {
      ...
      setCaseToken(restoreToken.trim()); setCaseData(restored); setSelected(restored.systems); setReportMode(restored.report_mode||'free')
  ...
  return <main>
    ...
    <section className="entry" id="entry"><div><p className="eyebrow">I. OPEN A CASE</p><h2>Bring your seven dossiers.</h2><p>每个体系分别上传 PDF、TXT 或图片。上传阶段只做文字识别与事实抽取，不生成性格、命运或建议。</p><RestoreForm id={restoreId} token={restoreToken} setId={setRestoreId} setToken={setRestoreToken} submit={restoreCase}/></div>
      <form onSubmit={startCase}><p className="optional-profile-note">基础资料均为可选项。报告里已有的信息不必重复填写；留空不会影响文件抽取。</p><label>姓名或昵称 <small>可选</small><input name="name" placeholder="可留空"/></label><div className="twocol"><label>出生日期 <small>可选</small><input name="date" type="date"/></label><label>出生时间 <small>可选</small><input name="time" type="time"/></label></div><label>出生地点 <small>可选</small><input name="place" placeholder="可留空"/></label><div className="twocol"><label>IANA 时区 <small>可选</small><input name="timezone" placeholder="例如 Asia/Shanghai；可留空"/></label><label>时间精度 <small>可选</small><select name="precision" defaultValue="unknown"><option value="unknown">未知 / 未填写</option><option value="exact">精确</option><option value="approximate">约略</option></select></label></div>
        <div className="key-panel"><label>DeepSeek API Key · 抽取、分析与辩论<input type="password" autoComplete="off" value={apiKey} onChange={e=>setApiKey(e.target.value)} placeholder="sk-...（只在页面内存）"/></label><div className="twocol"><label>模型<select value={model} onChange={e=>setModel(e.target.value)}><option value="deepseek-v4-flash">V4 Flash</option><option value="deepseek-v4-pro">V4 Pro</option></select></label><label>报告模式<select value={reportMode} onChange={e=>setReportMode(e.target.value as 'free'|'grounded')}><option value="free">实验自由模式 · 快速长文</option><option value="grounded">Grounded Skills · 引用审计</option></select></label></div>{reportMode==='free'&&<p className="mode-disclosure">实验自由模式：不读取七套 Skills、不要求 JSON 或 Rule ID；每室只调用一次 API。内容更接近 DeepSeek Chat，但不具备逐条证据审计，暂不开放后续结构化质询。</p>}{insecureRemote&&<p className="http-key-warning">⚠ 当前页面使用 HTTP。为了方便测试，Castle 仍允许填写并发送 Key，但传输过程不受 HTTPS 加密保护。请只短暂使用测试 Key，并尽快配置 HTTPS。</p>}<p>六份 AI-readable TXT/JSON 会在服务器本地直接解析，不消耗模型调用；其他 TXT 或文本 PDF 才由 DeepSeek 只抽取显式盘面事实。原文件与完整文本均不保存。用户确认后，报告与 Tribunal 只读取确认事实。服务器已配置 Key 时可留空。</p></div>
...
function ReportView({data,providerHeaders,setData,review,reportMode,setReportMode,apiKey,setApiKey,loading,setLoading,error,setError}:{data:CaseData;token:string;providerHeaders:()=>Record<string,string>;setData:(d:CaseData)=>void;review:()=>void;reportMode:'free'|'grounded';setReportMode:(v:'free'|'grounded')=>void;apiKey:string;setApiKey:(v:string)=>void;loading:string;setLoading:(v:string)=>void;error:string;setError:(v:string)=>void}) {
  ...
  const freeMode=data.report_mode==='free'
  return <section id="report" className="report"><header><div><p className="eyebrow">III. SEVEN GENERAL REPORTS</p><h2>The assembled testimony</h2></div><div className="badge skills_ready">{freeMode?'FREE TEST':'USER CONFIRMED'}<small>{freeMode?`${Object.values(data.reports).filter(report=>report.free_text).length} natural-language reports`:`${claims.length} admissible claims`}</small></div></header>{data.report_warnings?.map(item=><p className="warning disclosure" key={item}>{item}</p>)}
    <div className="regenerate-panel"><div><b>已确认事实会被保留</b><p>可以切换模式后直接重新生成，不需要重新上传或再次确认事实。</p></div><label>本次生成模式<select value={reportMode} onChange={e=>setReportMode(e.target.value as 'free'|'grounded')}><option value="free">实验自由模式 · 快速长文</option><option value="grounded">Grounded Skills · 引用审计</option></select></label>{!apiKey&&<label>DeepSeek API Key（服务器已配置时可留空）<input type="password" value={apiKey} onChange={e=>setApiKey(e.target.value)} placeholder="sk-...（只在页面内存）"/></label>}<div className="regenerate-actions"><button type="button" className="secondary" onClick={review} disabled={!!loading}>返回事实确认 / 重新上传</button><button type="button" onClick={regenerate} disabled={!!loading}>{loading||'按所选模式重新生成'}</button></div></div>
