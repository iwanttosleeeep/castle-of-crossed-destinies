...
export default function App() {
  ...
  function endCase() {
    if (!caseData) return
    const confirmed = window.confirm(`结束 ${caseData.case_id} 的本次浏览并重新开始？\n\n旧案仍保存在服务器；请确认你已经保存 Case ID 与恢复令牌。`)
    if (!confirmed) return
    setStage('entry'); setSelected(systems.map(([id])=>id)); setFiles({}); setApiKey(''); setModel('deepseek-v4-flash')
...
function ReportView({data,providerHeaders,setData,review,endCase,apiKey,setApiKey,loading,setLoading,error,setError}:{data:CaseData;providerHeaders:()=>Record<string,string>;setData:(data:CaseData)=>void;review:()=>void;endCase:()=>void;apiKey:string;setApiKey:(value:string)=>void;loading:string;setLoading:(value:string)=>void;error:string;setError:(value:string)=>void}) {
  ...
  return <section id="report" className="report"><header><div><p className="eyebrow">III. GENERAL REPORTS</p><h2>The assembled testimony</h2></div><div className="badge skills_ready">LIGHT SKILL<small>{completed} natural-language reports</small></div></header>{data.report_warnings?.map(item=><p className="warning disclosure" key={item}>{item}</p>)}
    ...
    <section className="case-closure"><p className="eyebrow">VI. CLOSE THE CASE</p><h2>Seal the record.</h2><p>下载完整 Markdown 案卷，或结束本次浏览并回到一份全新的空白案件。旧案不会从服务器删除。</p><div><button type="button" className="export-case" onClick={exportMarkdown} disabled={!!loading}>EXPORT COMPLETE DOSSIER · .MD</button><button type="button" className="end-case" onClick={endCase} disabled={!!loading}>结束庭审 · 重新开始</button></div></section>
  </section>
}
