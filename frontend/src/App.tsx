...
export default function App() {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (apiKey && !window.isSecureContext && !onLocalhost) throw new Error('BYOK_REQUIRES_HTTPS')
      const headers: Record<string,string> = {'Content-Type':'application/json', 'X-DeepSeek-Model':model}
      if (apiKey) headers['X-DeepSeek-Key'] = apiKey
      const response = await fetch('/api/reports', { method:'POST', headers, body:JSON.stringify({ systems:selected, facts_text:form.get('facts'), profile:{ display_name:form.get('name'), birth_date:form.get('date'), birth_time:form.get('time') || null, birthplace_text:form.get('place'), timezone_name:form.get('timezone'), time_precision:form.get('precision') } }) })
    ...
    } catch (caught) { setError(caught instanceof Error && caught.message === 'BYOK_REQUIRES_HTTPS' ? '为保护 API Key，请使用 HTTPS；当前 HTTP 部署请改用服务器端 .env。' : '无法连接到后端。请检查 FastAPI、反向代理或服务器日志。') } finally { setLoading(false) }
  }
  return <main>
    <nav><span className="site-title">THE CASTLE OF CROSSED DESTINIES</span><a href="#method">Method</a></nav>
    <section className="hero"><p className="eyebrow">SEVEN LOCAL SKILLS · AUDITABLE BY DESIGN</p><h1><em>THE CASTLE OF<br/>CROSSED DESTINIES</em></h1><p className="lede">一份出生资料，七套独立 Skill 证词。它们只解释你提供的盘面事实；每个结论都必须接受证据、矛盾与宽泛性审计。</p><div className="rule"/><p className="note">No chart-calculation API. AI analysis is sent only to the provider selected by the visitor.</p><div className="castle-card"><img src="/castle-card.jpg" alt="The Castle of Crossed Destinies card"/></div></section>
    <section className="entry" id="entry"><div><p className="eyebrow">I. CONDITIONS OF ENTRY</p><h2>Enter the castle</h2><p>Skill 不会自行排盘。请提供你已有的、可核验的盘面事实；时间精度会被带入每个敏感结论。</p></div><form onSubmit={submit}>
      ...
      <div className="key-panel"><label>DeepSeek API Key<input type="password" autoComplete="off" value={apiKey} onChange={event => setApiKey(event.target.value)} placeholder="sk-...（仅保存在当前页面内存）"/></label><label>模型<select value={model} onChange={event => setModel(event.target.value)}><option value="deepseek-v4-flash">DeepSeek V4 Flash</option><option value="deepseek-v4-pro">DeepSeek V4 Pro</option></select></label><p>Key 会经 HTTPS 传给 Castle 后端并立即转发给 DeepSeek；不会写入 localStorage、数据库或日志。关闭页面后即消失。</p></div>
    ...
    {report && <ReportView report={report}/>}<section className="method" id="method"><p className="eyebrow">THE METHOD</p><h2>Facts first. Skills second.</h2><p>系统不在后台自行排盘。每间 chamber 只读取用户确认且属于自己的事实；AI 输出必须同时回指事实 ID 与知识规则 ID。Tribunal 只比较中性证词，不读取角色化修辞。</p></section><footer>THE CASTLE OF CROSSED DESTINIES <span>Not advice. Not proof. A traceable conversation.</span></footer>
  </main>
}
